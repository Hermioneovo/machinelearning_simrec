...
class GeneralModel(BaseModel):
	...
	class Dataset(BaseModel.Dataset):
		def _get_feed_dict(self, index):
			...
			# 情况 1：评测阶段 + test_all = True（全物品排序，MovieLens 官方用法）
			if self.phase != 'train' and self.model.test_all:
				neg_items = np.arange(1, self.corpus.n_items)

			# 情况 2：存在预先生成的 neg_items（Grocery / Amazon 等）
			elif 'neg_items' in self.data:
				neg_items = self.data['neg_items'][index]

			# 情况 3：评测阶段但没有 neg_items（MovieLens-1M 的关键修复）
			else:
				# 动态随机采样负样本
				all_items = np.arange(1, self.corpus.n_items)
				neg_candidates = np.setdiff1d(all_items, [target_item])

				# num_neg 在模型参数里定义
				num_neg = getattr(self.model, 'num_neg', 99)

				# 防止物品数不足
				if len(neg_candidates) >= num_neg:
					neg_items = np.random.choice(
						neg_candidates, size=num_neg, replace=False
					)
				else:
